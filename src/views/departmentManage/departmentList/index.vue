...
export default {
  ...
  methods: {
    ...
    //删除明细
    table_del(row) {
      const departmentIdList = [row.departmentId]
      this.$API.department.department.delete.post({ departmentIdList }).then(() => {
        this.$message({
          message: '删除成功',
          type: 'success'
        })
    ...
    async batch_del() {
      try {
        ...
        await this.$API.department.department.delete.post({
          departmentIdList: this.selection.map(({ departmentId }) => departmentId)
        })
