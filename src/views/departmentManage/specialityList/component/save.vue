  ...
  <el-dialog :title="titleMap[mode]" v-model="visible" :width="500" destroy-on-close @closed="$emit('closed')">
    <el-form :model="form" :rules="rules" :disabled="mode=='show'" ref="dialogForm" label-width="100px">
      <el-form-item label="专业名称" prop="specialityName">
        <el-input v-model="form.specialityName" placeholder="请输入专业名称" clearable></el-input>
      </el-form-item>
  ...
  methods: {
    ...
    async submit() {
      ...
      try {
        ...
        await this.$API.department.speciality[operate].post(this.form)
        this.$message({
          type: 'success',
    ...
    //表单注入数据
    setData({ specialityId }) {
      this.$API.department.speciality.detail.get({ specialityId }).then(({data}) => {
        this.form = data
      }).catch(() => {
      })
    }
