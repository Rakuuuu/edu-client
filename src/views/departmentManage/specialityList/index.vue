...
<!--    </el-header>-->
    <el-main class="nopadding">
      ...
      >
        ...
        <el-table-column label="名称" prop="specialityName" width="180"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    //删除明细
    table_del(row) {
      const specialityIdList = [row.specialityId]
      this.$API.department.speciality.delete.post({ specialityIdList }).then(() => {
        this.$message({
          message: '删除成功',
          type: 'success'
        })
    ...
    async batch_del() {
      try {
        ...
        await this.$API.department.speciality.delete.post({
          specialityIdList: this.selection.map(({ specialityId }) => specialityId)
        })
